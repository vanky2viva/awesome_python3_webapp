...
async def create_pool(loop,**kw):
    logging.info('creat data base connection pool...')
    global __pool
...
async def execute(sql,args,autocommit):
    log(sql)
    async with __pool.get() as conn:
        if not autocommit:
            await conn.begin()
...
class ModelMetaclass(type):
    def __new__(cls,name,bases,attrs):
        ...
        attrs['__insert__'] = 'insert into `%s` (%s,`%s`) values(%s)' %(tableName,', ' .join(escaped_feilds),primary_key,create_args_string(len(escaped_fields)+1))
        attrs['__update__'] = 'update `%s` set %s where `%s`=?' % (tableName,', '.join(map(lambda f: '`%s`= ？' %(mappings.get(f).name or f),fields)),primary_key)
        attrs['__delete__'] = 'delete form `%s` where %s = ?' % (tableName, primary_key)
        return type.__new__(cls, name, bases,attes)
...
class Model(dict,metaclass=ModelMetaclass):
    ...
    async def findNumber(cls, selectField, where=None, args=None):
        ...
        sql = ['select %s _num_ from `%s`' %(selectField,__table__)]
        if where:
            sql.append('where')
            sql.append(where)
...
async def save(self):
    ...
    if row != 1:
        logging.wran('failed to insert  record:affected row: %s' % rows)
